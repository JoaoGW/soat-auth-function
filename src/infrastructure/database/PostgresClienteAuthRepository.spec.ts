...
describe('PostgresClienteAuthRepository', () => {
  it('consulta somente o documento normalizado com parâmetro SQL', async () => {
    const query = jest.fn().mockResolvedValue({ rows: [{ id: 'cliente-1', ativo: true }] });
    const repository = new PostgresClienteAuthRepository({ query } as unknown as Pick<Pool, 'query'>);
