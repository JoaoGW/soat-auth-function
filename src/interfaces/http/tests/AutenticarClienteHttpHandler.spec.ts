...
describe("criarAutenticarClienteHttpHandler", () => {
  ...
  it("retorna token para autenticação bem-sucedida", async () => {
    ...
    await expect(
      criarAutenticarClienteHttpHandler(useCase)(
        request({ cpf: "52998224725" }),
        {} as never,
      ),
    ).resolves.toMatchObject({
      status: 200,
      jsonBody: { accessToken: "jwt-do-cliente" },
    });
  });

  it.each(["invalido", "inexistente", "inativo"])(
    "não distingue CPF %s",
    async () => {
      const useCase = {
      ...
      await expect(
        ...
        status: 401,
        jsonBody: { mensagem: "CPF não autorizado" },
      });
    },
  );
  ...
  it("rejeita corpo JSON inválido", async () => {
    ...
    await expect(
      criarAutenticarClienteHttpHandler(useCase)(invalidRequest, {} as never),
    ).resolves.toMatchObject({
      status: 400,
